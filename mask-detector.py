...
net = jetson.inference.imageNet(
    argv=['--model=mask.onnx', '--labels=labels.txt', '--input_blob=input_0', '--output_blob=output_0'])
...
while True:
    ...
    font.OverlayText(img, img.width, img.height, "{:05.2f}% {:s}".format(
        confidence * 100, class_desc), 5, 5, font.White, font.Gray40)
    ...
    output.SetStatus("{:s} | Network {:.0f} FPS".format(
        net.GetNetworkName(), net.GetNetworkFPS()))
